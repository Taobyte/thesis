...
JOB="python main.py --multirun hydra/launcher=submitit_slurm dataset=ldalia lbw=$LBW pw=$PW model=linear normalization=global use_wandb=True experiment=endo_exo,endo_only seed=0,1,2"
sbatch --export=ALL --job-name="${DALIA_RUNS}_global"     -o "${DALIA_RUNS}_global_%j.out"     --time="$TIME" --wrap="$JOB"

JOB="python main.py --multirun hydra/launcher=submitit_slurm dataset=ldalia lbw=$LBW pw=$PW model=mole,msar,kalmanfilter,xgboost,gp,mlp,timesnet,simpletm,adamshyper,patchtst,timexer,gpt4ts,nbeatsx normalization=difference use_norm_dl=False use_wandb=True experiment=endo_exo,endo_only seed=0,1,2"
...
JOB="python main.py --multirun hydra/launcher=submitit_slurm dataset=lwildppg lbw=$LBW pw=$PW model=linear,msar,kalmanfilter,gpt4ts normalization=difference use_norm_dl=False use_wandb=True experiment=endo_exo,endo_only seed=0,1,2"
