...
JOB="python main.py --multirun hydra/launcher=submitit_slurm dataset=wildppg lbw=$LBW pw=$PW model=linear,mole,msar,mlp,adamsyhper,gpt4ts normalization=difference use_norm_dl=False use_wandb=True experiment=endo_exo,endo_only folds=fold_0,fold_1,fold_2 seed=0,1,2"
