...
JOB="python main.py --multirun hydra/launcher=slurm_eff use_prediction_callback=True dataset=dalia,wildppg,ieee lbw=$LBW pw=$PW model=timesnet,simpletm,adamshyper,patchtst,timexer,gpt4ts,nbeatsx normalization=global local_norm=local_z use_wandb=True experiment=endo_exo,endo_only folds=fold_0"
