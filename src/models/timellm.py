...
class Model(nn.Module):
    def __init__(
        ...
        if "llm_to_trsf" in model_id:
            # del self.tokenizer , self.llm_model, self.word_embeddings, self.mapping_layer, self.reprogramming_layer
            del self.output_projection
            self.basic_trsf = Encoder_TRSF(hidden_dim=d_model)

        if "llm_to_attn" in model_id:
            # del self.tokenizer , self.llm_model, self.word_embeddings, self.mapping_layer, self.reprogramming_layer
            del self.output_projection
            self.basic_attn = MultiHeadAttention(d_model=d_model)

        if "ori" not in model_id:
            self.output_projection = SimpleLinr(
                self.patch_nums * d_model,
                self.pred_len,
                head_dropout=dropout,
            )
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if "removeLLM" in self.model_id:
            ...
            dec_out = torch.reshape(dec_out, (B, n_vars, -1)).contiguous()
